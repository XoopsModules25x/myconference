...
class TagTagHandler extends XoopsPersistableObjectHandler
{
    ...
    public function getByItem($itemid, $modid = 0, $catid = 0)
    {
        ...
        $sql = 'SELECT o.tag_id, o.tag_term' . " FROM {$this->table_link} AS l " . " LEFT JOIN {$this->table} AS o ON o.{$this->keyName} = l.{$this->keyName} " . " WHERE  l.tag_itemid = {$itemid} AND l.tag_modid = {$modid}" . (empty($catid) ? '' : (' AND l.tag_catid=' . (int)$catid))
