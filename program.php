...
echo "<td class=\"itemHead\" valign=\"top\" ><a href=\"".XOOPS_URL."/modules/myconference/program.php\"><b>"._MA_PROGRAM."</b></a></td>";
echo "</tr></table>\n";
...
for ($d = $sdate; $d <= $edate; $d += $oneday) {
    ...
    $swidth = count(_MA_SCHEDULE);
    echo "<tr>\n<td align=center class=head width=$swidth>"._MA_SCHEDULE."</td>\n";
    $ntitle = array_reverse($title);
    foreach ($title as $tid => $v) {
        echo "<td align=center class=head>";
        if ($v) {
            echo "$v <a href=\"viewtrack.php?tid=$tid\"><sup>("._MA_INFOINITIAL.")</sup></a>";
        }
        echo " </td>\n";
    }
    echo "</tr>\n";
